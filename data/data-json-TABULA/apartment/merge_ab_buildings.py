...
APT_DIR = Path(__file__).resolve().parent
OUTPUT_PATH = APT_DIR / "ab_buildings_combined.json"


def load_buildings():
    buildings = []
    for fp in sorted(APT_DIR.glob("*.json")):
...
def build_combined(buildings):
    return {
        "metadata": {
            "source_directory": "data-json-TABULA/apartment",
